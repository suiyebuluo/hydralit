        ...
        if hasattr(hc, '__version__'):
            if hc.__version__ >= 104:
                ...
                self.session_state.selected_app = hc.nav_bar(menu_definition=menu_data, key="mainHydralitMenuComplex",
                                                             ...
                                                             login_name=login_nav, sticky_nav=self._navbar_sticky,
                                                             use_animation=self._navbar_animation,
                                                             hide_streamlit_markers=self._hide_streamlit_markings,
                                                             sticky_mode=self._navbar_mode)
        else:
            self.session_state.selected_app = hc.nav_bar(menu_definition=menu_data, key="mainHydralitMenuComplex",
                                                         home_name=self._home_app, override_theme=self._navbar_theme,
                                                         login_name=self._logout_label, sticky_nav=self._navbar_sticky)
        ...
        if self._complex_nav is None:
            if self._use_navbar:
                ...
                if self._login_app is not None:
                    ...
                    if self.session_state.current_user is not None:
                        self._logout_label = ['{} : {}'.format(self.session_state.current_user, self._logout_label[0]),
                                              None]
